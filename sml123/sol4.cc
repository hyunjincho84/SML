...
Expr eval_under_env(Expr e, std::map<string, Expr> env) {
    return std::visit(overload {
        ...
        [&](box < struct Call >& call) {
            /* TODO */
            // std::cout<<"call "<< toString(call)<<std::endl;
            Expr e1 = eval_under_env(call->funExpr, env);
            // std::cout << "a " <<toString(e1) << std::endl;
            // std::cout<<toString(e1)<<std::endl;
            Expr e2 = eval_under_env(call->actual, env);
            // std::cout <<"b " <<toString(e2) << std::endl;
            // std::cout<<toString(e2)<<std::endl;
            if(is<Closure>(e1))
            {
                ...
                // std::cout << "res " <<toString(e1) << std::endl;
                return res;
            }
            else 
            {
